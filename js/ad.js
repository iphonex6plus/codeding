/* url前缀 */
	...
	function load_Iframe(){
		...
		creatIframeList();
		//三个合并
		accountList.push([].concat(sum_li));
		accountList.push([].concat(sum_ding));
		accountList.push([].concat(sum_xiao));
	...
	function load_onePage(){
		...
		/*张明珠说娱乐*/
		allAccountList = allAccountList.concat(bjh_mingzhuyule);
		/*小太阳TYJ*/
	...
	function list_0(){
		var list = [bjh_tyjIds,bjh_zhitongwenhuaIds,bjh_yxdwzIds,bjh_tyjIds,bjh_xiaolai,bjh_mingzhuyule];
		/*账号中随机一个地址加载*/
	...
	function list_13(){
		var list =[];
		/*ding_sub*/
		list= list.concat(ding_sub);
		return list;
	}
	function list_14(){
		var list =[];
		/*li_sub*/
		list= list.concat(li_sub);
		return list;
	}
	function list_15(){
		var list =[];
	...
	function creatIframeList(){
		...
		/*大鱼号*/
		accountList.push(list_0());
		accountList.push(list_11());
		
		//莫小平谈影视
		var sub = [].concat(ydsIds);
		accountList.push(sub.concat(bjh_zhitongwenhuaIds));
		
	}
	function add_gzs_bjh(){
		
		//加洛特的小草帽
		accountList.push([].concat(bjh_zhitongwenhuaIds));
		//足坛第一视角
